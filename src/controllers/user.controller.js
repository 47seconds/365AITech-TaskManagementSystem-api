...
export const userLogin = asyncHandler(async (req, res) => {
  ...
  if (!(username && password))
    throw new ApiError(400, "provide login credentials");

  let loggedInUser = username.includes("@")
    ? await User.findOne({ email: username })
    : await User.findOne({ username: username });
  if (!loggedInUser)
    throw new ApiError(404, "no such user with given credentials");

  const isEnteredPasswordCorrect =
    await loggedInUser.isPasswordCorrect(password);
  if (!isEnteredPasswordCorrect)
    throw new ApiError(401, "incorrect password incorrect");
